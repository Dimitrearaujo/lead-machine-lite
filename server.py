...
# Config
# ─────────────────────────────────────────────────────────────
# Caminho absoluto garante que o .env é encontrado independente do working dir
...
def _call_chat_llm(context: str, extra_user: str = "") -> dict[str, Any]:
    ...
    # Monta mensagens sem assistant prefill (não suportado em claude-sonnet-4-6)
    ...
    try:
        ...
        log.error("Failed to parse LLM JSON: %s | raw=%s", e, raw[:300])
...
@app.get("/")
def serve_lp() -> FileResponse:
    if not PUBLIC_INDEX.exists():
        raise HTTPException(status_code=404, detail="lp_nao_encontrada")
    return FileResponse(str(PUBLIC_INDEX), media_type="text/html")
